# README - scripts (pilot-atendimento)

Este README descreve os scripts disponiveis em `pilot-atendimento/scripts/` e como usarlos.
...
- Se os scripts nao estiverem executaveis, rode: `chmod +x /home/ec2-user/pilot-atendimento/scripts/*.sh`.
...
- `chroma_list_collections.sh`
  ...
  - Uso recomendado (executa como usuario pilot):
    sudo -u pilot bash -lc 'cd /home/ec2-user/pilot-atendimento && ./scripts/chroma_list_collections.sh'
...
- `chroma_count_collection.sh`
  ...
  - Uso:
    sudo -u pilot bash -lc 'cd /home/ec2-user/pilot-atendimento && ./scripts/chroma_count_collection.sh [collection_name]'
...
- `chroma_ingest_base.sh`
  ...
  - Uso exemplo:
    sudo -u pilot bash -lc 'cd /home/ec2-user/pilot-atendimento && ./scripts/chroma_ingest_base.sh base/SEU_BASE_ID true'
...
- `chroma_backup.sh`
  ...
  - Uso (root ou usuario com permissao para ler `chroma_data`):
    /home/ec2-user/pilot-atendimento/scripts/chroma_backup.sh

- `retriever_query.sh`
  - O que faz: executa uma consulta de teste contra o retriever e imprime os resultados (usa o ambiente do projeto).
  - Uso exemplo:
    sudo -u pilot bash -lc 'cd /home/ec2-user/pilot-atendimento && ./scripts/retriever_query.sh "qual o horario de funcionamento?"'

Dicas de troubleshooting
- Se receber erros de permissao em `chroma_data` ou ao baixar modelos ONNX: verifique dono/permissoes com `ls -la /home/ec2-user/pilot-atendimento/chroma_data` e corrija com `sudo chown -R pilot:pilot /home/ec2-user/pilot-atendimento/chroma_data`.
